...
impl TryFrom<(Pair<'_>, Node)> for Node {
    type Error = ParseNodeError;
    fn try_from((pair, operand): (Pair<'_>, Node)) -> Result<Self, Self::Error> {
...
impl TryFrom<(Pair<'_>, Node, Node)> for Node {
    type Error = ParseNodeError;
    fn try_from((pair, lhs, rhs): (Pair<'_>, Node, Node)) -> Result<Self, Self::Error> {
    ...
    fn try_from(pair: Pair<'_>) -> Result<Self, Self::Error> {
        ...
        return match rule {
            ...
            Rule::UnaryExpression => {
                ...
                Node::try_from((operator, operand))
            }
            Rule::BinaryExpression => {
                ...
                let mut ret_val = Node::try_from((op, lhs, rhs))?;
                loop {
                    if let Some(operator) = pair.next() {
                        lhs = ret_val;
                        rhs = pair.next().try_into()?;
                        ret_val = Node::try_from((operator, lhs, rhs))?;
                    } else {
                        return Ok(ret_val);
                    }
                }
